...
const Section = ({ text } : { text: string }) => {
    return (
        <div>
            ...
            <div style={{ display: "flex", overflowX: "scroll", width: "100%", height: "200px", whiteSpace: "nowrap" }}>
                {data.map((chatbot) => (
                    <ChatList img={chatbot.img} name={chatbot.name} desc={chatbot.desc} key={chatbot.name} />
                ))}
            </div>
        </div>
    )
